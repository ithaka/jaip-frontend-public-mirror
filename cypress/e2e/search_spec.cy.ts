...
const resizeObserverNotificationErrRe =
  ...
  context('General', () => {
    context('Sorting', () => {
      context('When no search term is given', () => {
        beforeEach(() => {
          cy.intercept('POST', routes.search.basic, {
            fixture: 'search/no_term_given__response.json',
          }).as('search')
        ...
        it('Suppresses relevance sorting when no search term given', () => {
          ...
          cy.get('pep-pharos-dropdown-menu')
            .contains('Relevance', { matchCase: false })
            .should('not.exist')
        })

        it('Switches to relevance sort when switching from no term to a term', () => {
          cy.intercept('POST', routes.search.basic, {
            fixture: 'search/term_given__response.json',
          }).as('search')
          cy.visit('/search?term=&page=1')
      ...
      context('When there is a search term', () => {
        ...
        it('Lets users choose newest-first sort', () => {
          ...
          cy.intercept('POST', routes.search.basic, {
            fixture: 'search/term_given__newest__response.json',
          }).as('search')
          cy.wait('@search')
          cy.get('pep-pharos-button').contains('Sort By:', { matchCase: false }).click()
          cy.get('pep-pharos-dropdown-menu')
          ...
          cy.get('.document-title').first().should('contain.text', 'Rooted in DC')
        ...
        it('Lets users choose oldest-first sort', () => {
          ...
          // Interact with page to trigger new sort.
          cy.intercept('POST', routes.search.basic, {
            fixture: 'search/term_given__oldest__response.json',
          }).as('search')
          cy.wait('@search')
          cy.get('pep-pharos-button').contains('Sort By:', { matchCase: false }).click()
        ...
        it('Lets users choose relevance sort', () => {
          cy.visit('/search?term=mary+mcleod+bethune')
          cy.intercept('POST', routes.search.basic, {
            fixture: 'search/term_given__relevance__response.json',
          }).as('search')
          ...
          cy.get('.document-title').first().should('contain.text', 'MARY McLEOD BETHUNE')
    ...
    context('Filtering', () => {
      ...
      it('Lets users filter by publication year (max only)', () => {
        cy.intercept('POST', routes.search.basic, {
          fixture: `search/no_term_given__max_year__response.json`,
        }).as('search')
        cy.visit('/search?term=&page=1')
        cy.wait(['@searchPage', '@search'])
        cy.get('pep-pharos-input-group[name="search-within-max"]').scrollIntoView()
        ...
        cy.get('.document-title').first().should('have.text', '[INDEX OF AUTHORS]')
      })

      it('Lets users filter by publication year (min only)', () => {
        cy.intercept('POST', routes.search.basic, {
          fixture: `search/no_term_given__min_year__response.json`,
        }).as('search')
        cy.visit('/search?term=&page=1')
        cy.wait(['@searchPage', '@search'])
        cy.get('pep-pharos-input-group[name="search-within-min"]').scrollIntoView()
        ...
        cy.get('.document-title').first().should('have.text', 'Policing Suspicion')
      })

      it('Lets users filter by publication year (max and min)', () => {
        cy.intercept('POST', routes.search.basic, {
          fixture: `search/no_term_given__min_max_year__response.json`,
        }).as('search')
        cy.visit('/search?term=&page=1')
        cy.wait(['@searchPage', '@search'])
        cy.get('pep-pharos-input-group[name="search-within-min"]').scrollIntoView()
      ...
      it('Lets users filter by discipline', () => {
        cy.intercept('POST', routes.search.basic, {
          fixture: `search/no_term_given__discipline__response.json`,
        }).as('search')
        cy.visit('/search?term=&page=1')
        cy.wait(['@searchPage', '@search'])
      ...
      it('Lets users filter by journal', () => {
        cy.intercept('POST', routes.search.basic, {
          fixture: `search/no_term_given__journal__response.json`,
        }).as('search')
        cy.intercept('GET', routes.journals.get('africanamericanstudies-discipline'), {
        ...
        cy.wait('@afam')
        cy.get('pep-pharos-checkbox[value="abb70559-9665-3b42-aa8a-42535bf6f3c0"]').first().click()
